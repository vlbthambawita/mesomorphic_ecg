...
def get_top_pos_neg_segments_per_lead(impact: np.ndarray, window: int, stride: int, k_pos: int, k_neg: int) -> tuple[list[set[int]], list[set[int]]]:
    ...
    pos_segments: indices of top-k_pos segments with strongest POSITIVE contribution (Evidence for MI/Class 1).
    neg_segments: indices of top-k_neg segments with strongest NEGATIVE contribution (Evidence for NORM/Class 0).
    """
    n_leads, L = impact.shape
    T = (L - window) // stride + 1
    pos_res = []
...
def visualize_imn_to_pdf(model, dataset, device, pdf_path: str,
    ...
    If top_k_pos or top_k_neg > 0:
    - Red Box: Positive contribution segment (increases logit, evidence for MI).
    - Blue Box: Negative contribution segment (decreases logit, evidence for NORM).
    """
    model.eval()
    ...
    use_pos_neg_logic = (top_k_pos > 0 or top_k_neg > 0)
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            topk_str = ""
            if top_k_leads: topk_str += f" | top-{top_k_leads} leads"
            ...
            if show_heatmap:
                ...
                # Crop heatmap to relevant segments if using top-k
                all_top = set()
                if use_pos_neg_logic:
                ...
                seg_label = "Top Segments" if all_top else f"Segments (window={window}, stride={stride}, fs={sampling_rate}Hz)"
                ax0.set_xlabel(seg_label)
                
                expl_str = ""
                if use_pos_neg_logic:
                    expl_str = f" | Box: Red=Pos(MI), Blue=Neg(NORM)"

                ax0.set_title(f"IMN Single-Linear Explanation | {tag} | True={y_int} | {title_prob} | idx={idx}{topk_str}{expl_str}")
            ...
            for k, lead in enumerate(lead_indices):
                ...
                if is_top:
                    ax.set_facecolor((1, 0.95, 0.9) if (y_int==1) else (0.95, 0.97, 1)) # Light shade hint
            ...
            axf.axis("off")
            footer = f"IMN Single-Linear Feature Attribution. L1 Reg={lambda_l1}"
            if use_pos_neg_logic:
                footer += " | Red (+): Increases Logit (MI Evidence). Blue (-): Decreases Logit (NORM Evidence)."
...
def visualize_ecg_with_ecg_plot(
    ...
    if ecg.ndim == 3:
        ecg = ecg[0]
    assert ecg.ndim == 2 and ecg.shape[0] == 12, "ecg must be (12, L)"
    plot_kwargs = dict(sample_rate=sample_rate, title=title, columns=columns)
    if lead_names is not None: plot_kwargs["lead_index"] = lead_names
    if style is not None: plot_kwargs["style"] = style
...
def visualize_ecg_batch_with_ecg_plot(ecg_batch, sample_rate=500, out_dir="ecg_plots", n_samples=5, title_prefix="ECG", random_indices=False, seed=42):
    if not ECG_PLOT_AVAILABLE: raise ImportError("ecg_plot not installed")
    os.makedirs(out_dir, exist_ok=True)
    N = ecg_batch.shape[0]
    ...
    for idx in indices:
        ...
        visualize_ecg_with_ecg_plot(ecg_batch[idx], sample_rate=sample_rate, title=f"{title_prefix} {idx}", save_path=fname)
...
def _draw_ecg_plot_style(ax, ecg: np.ndarray, sample_rate: int, lead_names: list,
    ...
    for c in range(columns):
        for i in range(rows):
            if c * rows + i >= leads: break
            t_lead = lead_order[c * rows + i]
            y_offset = -(row_height / 2) * ceil(i % rows)
            x_offset = secs * c if c > 0 else 0
            sep_h = 0.12 * (row_height / 0.5)
            if c > 0:
                ax.plot([x_offset, x_offset], [ecg[t_lead][0] + y_offset - sep_h, ecg[t_lead][0] + y_offset + sep_h], linewidth=line_width, color=color_line)
            ax.text(x_offset + 0.07, y_offset - row_height * 0.3, lead_names[t_lead], fontsize=7 * (display_factor ** 0.5))
            ax.plot(np.arange(0, len(ecg[t_lead]) * step, step) + x_offset, ecg[t_lead] + y_offset, linewidth=line_width, color=color_line)
...
def _draw_important_patches_on_ecg(
    ...
    full_secs = (signal_len / sampling_rate) if signal_len else (max((Tseg - 1) * stride + window, window) / sampling_rate if Tseg > 0 else window / sampling_rate)
    secs = full_secs / 2 if half_signal else full_secs
    ylo, yhi = ax.get_ylim()
    rows = int(ceil(n_leads / columns))
    band_h = row_height / 2

    def _draw_rect(x_lo, y_lo, w, h, alpha):
    ...
    for t in range(Tseg):
        ...
        if half_signal and end_sec > secs: continue
        w = end_sec - start_sec
        ...
        if per_lead:
            for lead_idx in range(n_leads):
                ...
                if col == 0: _draw_rect(start_sec, y_lead_lo, w, h_lead, alpha)
                else: _draw_rect(secs + start_sec, y_lead_lo, w, h_lead, alpha)
        else:
            ...
            if columns > 1 and end_sec <= secs: _draw_rect(secs + start_sec, ylo, w, h, alpha)
...
def visualize_ecg_with_imn_heatmap_to_pdf(
    ...
    for idx in indices:
        ...
        # Per-sample top-k segments logic
        ...
        seg_hm_legacy = seg_hm[lead_indices].copy()

        if use_pos_neg_logic:
            for i, lead_idx in enumerate(lead_indices):
                if pos_seg_per_lead:
                    for t in pos_seg_per_lead[lead_idx]: seg_hm_pos[i, t] = 1.0
                if neg_seg_per_lead:
                    for t in neg_seg_per_lead[lead_idx]: seg_hm_neg[i, t] = 1.0
        elif top_seg_per_lead is not None:
            for i, lead_idx in enumerate(lead_indices):
                segs = top_seg_per_lead[lead_idx]
                for t in range(Tseg):
                    seg_hm_legacy[i, t] = 1.0 if t in segs else 0
        ...
        if show_heatmap:
            ...
            seg_label = "Top Segments" if all_top else f"Segments (w={window}, s={stride}, fs={sampling_rate}Hz)"
        ...
        if use_pos_neg_logic:
            ...
            _draw_important_patches_on_ecg(
                ax_ecg, seg_hm_legacy, window, stride, sampling_rate,
                shade_color=shade_color, signal_len=Lsig, half_signal=half_ecg,
                draw_bbox=use_full_12, per_lead=use_full_12, row_height=row_height,
            )

        if use_full_12:
            expl = "Red=Pos(MI), Blue=Neg(NORM)" if use_pos_neg_logic else "boxed=top-k segs"
            ...
            shade_str = "top-k segments" if all_top else "IMN important regions"
