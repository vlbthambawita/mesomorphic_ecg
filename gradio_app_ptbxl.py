"""
Gradio app for ECG Patch IMN with PTB-XL: load ECG from dataset path, show clinical
notes and ground truth, then predict, visualize, and ablate.

Usage: pip install gradio wfdb && python gradio_app_ptbxl.py
"""
...
build_superclasses = train.build_superclasses
...
def ablate_and_recompute(ps: torch.Tensor, b: torch.Tensor, T: int,
    top_lead_idx: list[int], top_patch_idx: list[int],
    n_remove_leads: int, n_remove_patches: int) -> float:
    ps = ps.clone()
    P = ps.shape[1]
    lead_dim = P // 12
...
def build_fig(x_np: np.ndarray, heatmap: np.ndarray, window: int, stride: int,
    top_leads: Optional[list[int]], top_patches: Optional[list[int]],
    removed_leads: Optional[list[int]], removed_patches: Optional[list[tuple[int, int]]],
    T: int, pred: str, prob: float, prob_abl: Optional[float]) -> plt.Figure:
    ...
    for lead in range(12):
        ...
        ax.set_xlim(0, L - 1)
        ax.set_ylabel(f"{LEAD_NAMES[lead]} {lead_pct[lead]:.1f}%", rotation=0, labelpad=20, va="center")
        ylo, yhi = ax.get_ylim()
        if top_leads and lead in top_leads:
            ax.axhspan(ylo, yhi, alpha=0.15, color="gold", zorder=0)
        if lead in rem_lead_set:
            ax.add_patch(mpatches.Rectangle((0, ylo), L - 1, yhi - ylo, fill=False, edgecolor="red", linewidth=2.5, zorder=10))
        for ti in range(T):
            a = float(hm[lead, ti])
            ...
            if is_rem:
                ...
                ax.add_patch(mpatches.Rectangle((s, ylo), e - s, yhi - ylo, fill=False, edgecolor="red", linewidth=2, zorder=10))
            elif hi:
...
def load_ptbxl_meta(ptbxl_path: str, sampling_rate: int):
    ...
    for i, (idx, row) in enumerate(Y_test.iterrows()):
        ...
        records.append({"ecg_id": int(idx), "filename": fn, "report": report, "gt": gt, "y": float(y_test[i]), "age": age, "sex": sex})
...
def on_load_ptbxl(ptbxl_path: str, sampling_rate: int, state: Optional[dict]):
    ...
    return f"Loaded {len(records)} test records (MI vs NORM).", gr.update(choices=choices, value=value), state, report, gt
...
def predict_ptbxl(ckpt_path: str, ptbxl_path: str, sampling_rate: int, record_choice: str, state: Optional[dict],
    topk_leads: int, topk_patches: int, remove_leads: bool, n_remove_leads: int, remove_patches: bool, n_remove_patches: int):
    ...
    if x.shape[1] != signal_len:
        return f"ECG length {x.shape[1]} != model {signal_len}. Use matching sampling rate.", None, report, gt, "—", "—", "—"
    x = zscore_per_lead(x)
    ...
    fig = build_fig(x, hm, window, stride, top_leads or None, top_patches or None,
        removed_leads or None, removed_patches or None, T, pred, prob_val, prob_abl)
    ...
    summary = f"**{pred}** | P(MI) = {prob_val:.3f} | Ground truth: **{gt}**"
    return summary, fig, report, gt, tl, tp, abl


def main():
    demo = gr.Blocks(title="ECG Patch IMN – PTB-XL", theme=gr.themes.Soft())
    with demo:
        gr.Markdown("# ECG Patch IMN – PTB-XL\nLoad ECGs from **PTB-XL** path. View **clinical notes** and **ground truth**, then run prediction, segment/lead importance, and ablation.")
        with gr.Row():
            ptbxl_path = gr.Textbox(label="PTB-XL path", value=DEFAULT_PTBXL if os.path.isdir(DEFAULT_PTBXL) else "", placeholder="path/to/ptb-xl/1_0_1")
        ...
        with gr.Row():
            record_dd = gr.Dropdown(label="Test record (ecg_id | GT | age sex)", choices=[], value=None)
        with gr.Row():
            clinical_notes = gr.Textbox(label="Clinical notes (report)", value="", lines=4, max_lines=8, interactive=False)
            ground_truth = gr.Textbox(label="Ground truth", value="—", interactive=False)
        load_btn.click(fn=on_load_ptbxl, inputs=[ptbxl_path, fs, ptbxl_state], outputs=[load_status, record_dd, ptbxl_state, clinical_notes, ground_truth])
        record_dd.change(fn=on_select_record, inputs=[record_dd, ptbxl_state], outputs=[clinical_notes, ground_truth])
        ...
        run_btn.click(
            fn=predict_ptbxl,
            inputs=[ckpt, ptbxl_path, fs, record_dd, ptbxl_state, topk_leads, topk_patches, remove_leads, n_remove_leads, remove_patches, n_remove_patches],
            outputs=[out_summary, out_plot, out_notes, out_gt, out_leads, out_patches, out_abl],
        )
        gr.Markdown("PTB-XL path = dir with ptbxl_database.csv, scp_statements.csv. Test fold only. Red=MI, Blue=NORM. Red boxes=removed.")
