...
# -----------------------
# Option B Model:
# Global encoder reads full ECG -> g
# Hyper outputs scalar gates a [B,P]
# tokens -> token_head -> patch_logit [B,P]
# patch_scores = a * patch_logit
# logit = sum(patch_scores) + bias(g)
# -----------------------
class ECGPatchIMN_OptionB(nn.Module):
    def __init__(self, signal_len: int, window: int, stride: int, token_dim: int = 64, hyper_hidden: int = 256, dropout: float = 0.1):
        ...
        # Hypernetwork for scalar gates a (per patch)
        self.hyper_a = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hyper_hidden, self.P),   # a: [B,P]
        )

        # Global token-to-logit head (shared across patches)
        self.token_head = nn.Linear(token_dim, 1, bias=False)  # patch_logit = <w, token>

        # Instance-specific bias from g
        self.bias = nn.Linear(token_dim, 1, bias=True)

    def forward(self, x):
        """
        x: [B,12,L]
        returns: logit [B], a [B,P], tokens [B,P,D], T
        """
        ...
        a = self.hyper_a(g)                             # [B,P]

        patch_logit = self.token_head(tokens).squeeze(-1)   # [B,P]
        patch_scores = a * patch_logit                      # [B,P]

        b = self.bias(g).squeeze(-1)                        # [B]
        logit = patch_scores.sum(dim=1) + b                 # [B]

        return logit, a, tokens, T

    @torch.no_grad()
    def explain(self, x):
        logit, a, tokens, T = self.forward(x)
...
def train_one_epoch(model, loader, opt, device, l1_lambda: float, pos_weight: float):
    ...
    for x, y in loader:
        ...
        # Option B returns (logit, a, tokens, T)
        logit, a, tokens, T = model(x)

        bce = F.binary_cross_entropy_with_logits(logit, y, pos_weight=pos_w)

        # Sparsity: encourage sparse gates a (patch-level sparsity)
        l1 = a.abs().mean()
