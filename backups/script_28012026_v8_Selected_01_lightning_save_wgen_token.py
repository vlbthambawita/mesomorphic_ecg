...
class ECGPatchIMN(nn.Module):
    ...
    def __init__(
        ...
        # ✅ Hypernet now outputs (P + 1): patch weights + bias
        ...
        # ✅ Shared linear head over token_dim to get one scalar per patch
        # (no bias here; bias comes from hypernet as requested)
    ...
    def forward(self, x, verbose: bool = None):
        ...
        # token_head outputs [B,P,1] -> squeeze -> [B,P]
    ...
    def explain(self, x):
        ...
        # Per-patch signed contribution that sums to the logit (without bias)
        # logit = sum_p Wpatch_p * patch_score_p + b
    ...
    def explain_with_topk(self, x, topk_leads: int = None, topk_patches: int = None):
        ...
        # Same contribution definition as in explain()
        patch_contrib = (Wpatch * patch_score)           # [B,P]
        heatmap = patch_contrib.view(x.size(0), 12, T)   # [B,12,T]
        ...
        if topk_patches is not None and topk_patches > 0:
            ...
            patch_abs = patch_contrib.abs()  # [B, P]
            _, top_patch_indices = torch.topk(patch_abs, k=min(topk_patches, patch_contrib.size(1)), dim=1)  # [B, topk_patches]
