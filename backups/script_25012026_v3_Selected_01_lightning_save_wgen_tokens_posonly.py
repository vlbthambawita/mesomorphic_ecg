"""
ECG Patch IMN (Lightning) — positive-only patch contributions variant.

This script is identical to `script_25012026_v3_Selected_01_lightning_save_wgen_tokens.py`
except that all patch contributions (heatmaps) are rectified to be non-negative:
  - Contributions that push toward NORM (negative in the original script) are set to 0.
  - Visualizations therefore show only positive (MI-pushing) contributions.
"""
import argparse
import ast
import os
...
class ECGPatchIMN(nn.Module):
    ...
    In this variant, explanation/heatmap methods rectify patch contributions to be non-negative.
    """
    ...
    def explain(self, x):
        ...
        patch_scores = patch_scores.clamp_min(0.0)       # keep only positive contributions
    ...
    def explain_with_topk(self, x, topk_leads: int = None, topk_patches: int = None):
        ...
        if topk_leads is not None and topk_leads > 0:
            # Compute per-lead importance (sum of contributions, already >=0)
        ...
        if topk_patches is not None and topk_patches > 0:
            # Get top-k patches by contribution value (>=0)
            patch_vals = patch_scores  # [B,P]
            _, top_patch_indices = torch.topk(patch_vals, k=min(topk_patches, patch_scores.size(1)), dim=1)  # [B, topk_patches]
...
def collect_wgen_tokens(model, loader, device, max_samples: int = None):
    ...
    NOTE: Wgen/tokens are raw; positive-only rectification is applied only when computing heatmaps.
    """
    model.eval()
...
def visualize_pos_neg_to_pdf(model, dataset, device, pdf_path: str, sampling_rate: int,
                             ...
                             lead_names=None):
    """
    Saves a multi-page PDF with non-negative patch contributions only:
      - First N positives (MI=1) and M negatives (NORM=0), or random if random_pick=True.
      - Each page: heatmap + 12 lead plots with shaded positive contributions.
    """
    model.eval()
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            hm = heatmap.squeeze(0).detach().cpu().numpy()  # [12,T], >=0
            ...
            axf.axis("off")
            axf.text(0, 0.5, "Red=positive patch contribution toward MI (negative contributions clipped to 0, normalized per record).",
                     fontsize=10)
...
def visualize_topk_leads_patches_to_pdf(model, dataset, device, pdf_path: str, sampling_rate: int,
                                       ...
                                       lead_names=None):
    """
    Visualizes samples with top-k leads and patches highlighted using non-negative contributions.
    """
    model.eval()
    if lead_names is None:
        lead_names = ["I","II","III","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6"]
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            axf = fig.add_subplot(gs[13, 0])
            axf.axis("off")
            info_text = "Red=positive patch contribution toward MI (negative contributions clipped to 0, normalized per record).\n"
...
def visualize_uncertainty_to_pdf(model, dataset, device, pdf_path: str, sampling_rate: int,
                                 ...
                                 lead_names=None):
    """
    Visualizes uncertainty from Monte Carlo dropout sampling with non-negative contributions.
    """
    model.eval()
    if lead_names is None:
        lead_names = ["I","II","III","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6"]
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            mean_hm = mean_heatmap.squeeze(0).detach().cpu().numpy()  # [12,T], >=0
            ...
            ax0.set_yticklabels(lead_names)
            ax0.set_xlabel(f"Segments (window={window}, stride={stride}, fs={sampling_rate}Hz)")
            ax0.set_title(f"{tag} sample | true={int(y.item())} | P(MI)={mean_prob_val:.3f}±{std_logit_val:.3f} | idx={idx} | Mean Positive Contributions")
            ...
            for lead in range(12):
                ...
                mean_contrib = mean_hm_disp[lead]  # [T], >=0
                ...
                for t in range(T):
                    ...
                    if alpha_mean > 0:
                        color = "red"
                        ax.axvspan(start, end, alpha=alpha_mean, color=color, linewidth=0, label='Mean positive contribution' if t == 0 else '')
            ...
            axf.axis("off")
            info_text = f"Mean contributions: Red=positive contribution toward MI (negative clipped to 0).\n"
...
def main():
    ...
    # Visualization
    parser.add_argument("--viz_pdf", type=str, default="viz_mi_vs_norm_test_posonly.pdf", help="Output PDF filename")
    ...
    parser.add_argument("--viz_topk_pdf", type=str, default="viz_topk_test_posonly.pdf", help="Output PDF filename for top-k visualization")
    ...
    parser.add_argument("--viz_uncertainty_pdf", type=str, default="viz_uncertainty_test_posonly.pdf", help="Output PDF filename for uncertainty visualization")
    ...
    print(f"\n📊 Saving per-lead positive-only visualizations to: {pdf_path}")
    ...
    if args.viz_topk:
        ...
        print(f"\n📊 Saving top-k leads/patches positive-only visualizations to: {topk_pdf_path}")
    ...
    if args.viz_mc_dropout:
        ...
        print(f"\n📊 Saving Monte Carlo dropout uncertainty positive-only visualizations to: {uncertainty_pdf_path}")
