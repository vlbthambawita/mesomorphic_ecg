...
This app:
  ...
  - Runs the IMN model (script_02022026_v7_IMN_GM_2_with_transition_net.py).
  - Visualizes feature attribution (Impact = w * x) aggregated by segments.
  - User can select window and stride to explore different segment granularities.
  - Visualizes top-k leads and top-k segments (highlighted in gold/lime).
  - Optional ablation: remove top leads/segments and recompute P(MI).
"""
...
import script_02022026_v7_IMN_GM_2_with_transition_net as imn_script
...
# Default checkpoint for norm_vs_mi at 100Hz
DEFAULT_CKPT = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "runs/imn_ecg_transition_net_GM_100Hz/norm_vs_mi/2026-02-02_11-56-53/best-imn-epoch=24-val_auc=0.9591.ckpt",
)
...
def load_model(ckpt_path: str, device: Optional[str] = None):
    """Load and cache the IMN Lightning model from a checkpoint."""
...
def ablate_and_recompute_imn(
    ...
    """
    Ablation: "Remove a lead" = we don't have that signal, so it contributes
    to NEITHER class. Zero weights for both classes at removed leads/segments,
    then recompute both logits. This gives balanced, interpretable results.
    (Zeroing only one class caused extreme P(MI)=0 when that lead dominated.)
    gen_w: [1, num_classes, 12, L], x_t: [1, 12, L].
    Returns P(class 1) = P(MI) after ablation.
    """
    w_all = gen_w[0].clone()  # [num_classes, 12, L]

    # Remove whole leads: zero for BOTH classes (lead no longer contributes)
    if n_remove_leads > 0 and top_leads:
        for li in top_leads[:n_remove_leads]:
            w_all[:, li, :] = 0.0

    # Remove segments: zero for BOTH classes
    ...
    # Recompute BOTH logits with modified weights
    x_exp = x_t[0]  # [12, L]
    w_all = w_all.to(x_exp.device)
    new_logits = (w_all * x_exp.unsqueeze(0)).sum(dim=(1, 2)) + gen_b[0, :, 0]
    prob_mi = float(torch.softmax(new_logits.unsqueeze(0), dim=1)[0, 1].item())
    return prob_mi
...
def build_fig_imn(
    ...
    """
    Build a matplotlib figure for IMN feature attribution visualization.
    seg_hm: [12, T] segment heatmap from imn_weights_to_segments.
    """
    ...
    cmap = "Blues" if viz_negative_class else "Reds"
    shade_color = "blue" if viz_negative_class else "red"
    weight_label = "NORM" if viz_negative_class else pos_class_name
    ...
    prob_str = f"P({pos_class_name})={prob_pos:.3f}" if not viz_negative_class else f"P(NORM)={1 - prob_pos:.3f}"
    title = f"IMN Intrinsic Explanation | {pred} | {prob_str} | Weights={weight_label}"
    if prob_abl is not None:
        p_str = f"{prob_abl:.4f}" if prob_abl < 0.001 else f"{prob_abl:.3f}"
        title += f" -> Ablated P(MI) = {p_str}"
    ax0.set_title(title)
    ...
    axf.axis("off")
    leg = (
        f"IMN Feature Attribution: |w(x)·x| aggregated by segment (window={window}, stride={stride}). "
        "Gold/Lime = top leads/segments by signed contribution (highest positive = most evidence for class). "
    )
    if rem_lead_set or rem_seg_set:
        leg += "Red boxes = removed (ablation). "
    if viz_negative_class:
        leg += "Showing NORM (class 0) weights."
...
def predict_imn(
    ...
    """
    Run IMN inference on a selected PTB-XL record and visualize feature attribution
    with user-selected window and stride. Optionally show top-k leads/segments and
    ablate them to see effect on prediction.
    """
    ...
    with torch.no_grad():
        ...
        prob_pos = float(prob[0, 1].item())
        y_int = int(rec["y"])

        # For NORM samples when viz_negative_class: use class 0 weights
    ...
    # Aggregate into segments using user-selected window and stride
    seg_hm = imn_weights_to_segments(impact, window=window, stride=stride)
    T = seg_hm.shape[1]
    L = x_np.shape[1]

    # Top-k leads: rank by SIGNED contribution (impact). Leads with highest
    # positive impact push toward the predicted class; removing them decreases
    # P(MI). Using |impact| would include negative contributors (evidence
    # against MI); removing those would incorrectly increase P(MI).
    ...
    if (nr or ns) and (top_leads or top_segments):
        prob_abl = ablate_and_recompute_imn(
            gen_w,
            x_t,
            gen_b,
            top_leads,
    ...
    pred = "MI" if prob_pos >= 0.5 else "NORM"
    pos_class_name = "MI"
    ...
    abl = f"Ablated P(MI) = {_fmt_prob(prob_abl)}" if prob_abl is not None else "—"
    summary = (
        f"**{pred}** | P(MI) = {prob_pos:.3f}"
...
def main():
    demo = gr.Blocks(
        title="ECG IMN – PTB-XL (Transition Net)",
        theme=gr.themes.Soft(),
    )
    with demo:
        gr.Markdown(
            "# ECG IMN – PTB-XL (Transition Net)\n"
            "Load ECGs from **PTB-XL** path. View **clinical notes** and **ground truth**, "
            "then run IMN prediction with **feature attribution** (Impact = w·x). "
        ...
        # Model & prediction
        gr.Markdown("### Model & IMN prediction")
        with gr.Row():
            ckpt = gr.Textbox(
                label="Checkpoint path (IMN transition net)",
                value=DEFAULT_CKPT if os.path.isfile(DEFAULT_CKPT) else "",
                placeholder="path/to/best-imn.ckpt",
            )
        ...
        with gr.Row():
            viz_negative_class = gr.Checkbox(
                label="For NORM samples: show NORM (class 0) weights instead of MI weights",
                value=False,
            )

        gr.Markdown(
            "### Top-k leads & segments\n"
            "Ranked by **signed** contribution (w·x): highest positive = most evidence for the class. "
            "Removing them should decrease P(MI) for MI-predicting leads."
        )
        with gr.Row():
        ...
        gr.Markdown(
            "IMN generates weights w(x) per sample point. Attribution = w·x. "
            "Window/stride control segment aggregation. Top-k leads/segments are highlighted (gold/lime). "
            "Enable ablation to remove top contributors and see how P(MI) changes. "
            "Default checkpoint: norm_vs_mi at 100Hz (signal_len=1000). Match sampling rate to your checkpoint."
        )
