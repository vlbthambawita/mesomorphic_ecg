...
# IMN v2 (Transition Net, 2-class CE) - Inference + visualization.
# Usage: bash infer_imn_v2_ce.sh [window] [stride] [leads] [heatmap_height] [ecg_height]
...
for rate in "${RATES[@]}"; do
  for task in "${TASKS[@]}"; do
    ...
    [[ -z "${CKPT_PATH}" ]] && { echo "  [WARN] No best-imn-*.ckpt found (skipping)"; continue; }
    echo "  Using checkpoint: ${CKPT_PATH}"
    python models/imn_v2_transition_ce.py \
      ...
      --viz_negative_class --viz_pdf "viz_${task}_imn_inference.pdf" \
      --wandb_project mesormorphic_ecg ${LEADS_ARG} ${HEATMAP_HEIGHT_ARG} ${ECG_HEIGHT_ARG}
