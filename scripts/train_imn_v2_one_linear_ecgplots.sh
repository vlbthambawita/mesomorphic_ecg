...
# IMN v2 (single linear) - Training with ECG+IMN heatmap plots enabled.
# Quick test: --epochs 1. Output: runs/imn_ecg_transition_net_GM_one_linear_test_ecg_plots_*Hz
...
for rate in "${RATES[@]}"; do
  for task in "${TASKS[@]}"; do
    ...
    python models/imn_v2_transition_one_linear.py \
      ...
      --epochs 1 --batch_size 64 --lambda_l1 1e-4 \
