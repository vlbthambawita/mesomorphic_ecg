...
# IMN v2 (single linear) - Inference with ECG+IMN heatmap layout (compact plots).
# Usage: bash infer_imn_v2_one_linear_ecgplots.sh [window] [stride] [leads] [heatmap_height] [ecg_height]
...
for rate in "${RATES[@]}"; do
  for task in "${TASKS[@]}"; do
    ...
    python models/imn_v2_transition_one_linear.py \
      ...
      --n_pos_viz 1 --n_neg_viz 1 ${WINDOW_ARG} ${STRIDE_ARG} --viz_random \
      --viz_pdf "viz_${task}_imn_inference.pdf" --wandb_project mesormorphic_ecg \
      --one_per_file --viz_ecg_plot --viz_ecg_plot_n 5 \
      ${LEADS_ARG} ${HEATMAP_HEIGHT_ARG} ${ECG_HEIGHT_ARG}
