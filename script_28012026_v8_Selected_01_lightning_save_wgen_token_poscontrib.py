...
Variant with **non-negative patch contributions** in explanation utilities:
- `explain`, `explain_with_topk`, and Monte Carlo dropout explanations use
  the **absolute value** of per-patch contributions so that all patch values
  in the returned heatmaps are >= 0.
"""
import argparse
import ast
import os
...
# -----------------------
# Model: Patch-wise IMN (binary) with overlap support (PATCH-WEIGHT VERSION)
# -----------------------
class ECGPatchIMN(nn.Module):
    ...
    - Pool tokens -> g
    - Hypernet(g) -> instance-specific PATCH weights over tokens (P) + bias
    - Shared linear head over token_dim gives per-patch scalar scores
    - logit = sum_p Wpatch_p * score_p + b
    """
    def __init__(
        self,
        signal_len: int,
        ...
        # Hypernet outputs (P + 1): patch weights + bias
        ...
        # Shared linear head over token_dim to get one scalar per patch (no bias here)
        self.token_head = nn.Linear(token_dim, 1, bias=False)
    ...
    def forward(self, x, verbose: bool = None):
        ...
        returns:
          logit: [B]
          Wpatch: [B,P]          (patch-wise importance weights)
          tokens: [B,P,D]
          patch_score: [B,P]     (scalar score per patch from shared linear head)
        ...
        # Patch-wise weights + bias
        Wpatch = out[:, :-1]                            # [B,P]
        b = out[:, -1]                                  # [B]

        # Per-patch scalar score using shared linear head over D
        patch_score = self.token_head(tokens).squeeze(-1)  # [B,P]

        if verbose:
            print(f"[Forward] Wpatch shape: {Wpatch.shape}, b shape: {b.shape}")
            print(f"[Forward] patch_score shape: {patch_score.shape}")
            print(f"[Forward] Wpatch*patch_score shape: {(Wpatch * patch_score).shape}")

        # logit = sum_p Wpatch_p * patch_score_p + b
        logit = (Wpatch * patch_score).sum(dim=1) + b   # [B]

        if verbose:
            print(f"[Forward] Final output: logit shape: {logit.shape}")
            print(f"[Forward] Final output: logit shape: {logit.shape}, Wpatch shape: {Wpatch.shape}, tokens shape: {tokens.shape}, T: {T}")

        return logit, Wpatch, tokens, patch_score, T
    ...
    def explain(self, x):
        ...
        returns:
          prob: [B]
          heatmap: [B,12,T] **non-negative** contributions per lead×segment
        """
        # forward() returns: logit, Wpatch, tokens, patch_score, T
        logit, Wpatch, _tokens, patch_score, T = self.forward(x)
        prob = torch.sigmoid(logit)

        # Per-patch contribution magnitude (non-negative)
        # logit = sum_p Wpatch_p * patch_score_p + b
        patch_contrib = (Wpatch * patch_score)           # [B,P]
        patch_contrib = patch_contrib.abs()              # make non-negative
        heatmap = patch_contrib.view(x.size(0), 12, T)   # [B,12,T]
        return prob, heatmap
    ...
    def explain_with_topk(self, x, topk_leads: int = None, topk_patches: int = None):
        ...
        returns:
          prob: [B]
          heatmap: [B,12,T] **non-negative** contributions per lead×segment
          top_lead_indices: [B, topk_leads] indices of top-k leads (by magnitude)
          top_patch_indices: [B, topk_patches] indices of top-k patches (flattened lead×time, by magnitude)
        """
        logit, Wpatch, _tokens, patch_score, T = self.forward(x)
        prob = torch.sigmoid(logit)
        ...
        patch_contrib = (Wpatch * patch_score)           # [B,P]
        patch_abs = patch_contrib.abs()                  # [B,P], non-negative
        heatmap = patch_abs.view(x.size(0), 12, T)       # [B,12,T]

        top_lead_indices = None
        top_patch_indices = None

        if topk_leads is not None and topk_leads > 0:
            # Compute per-lead importance (sum of magnitudes)
            lead_importance = heatmap.sum(dim=2)  # [B, 12]
        ...
        if topk_patches is not None and topk_patches > 0:
            # Get top-k patches by magnitude
            _, top_patch_indices = torch.topk(patch_abs, k=min(topk_patches, patch_abs.size(1)), dim=1)  # [B, topk_patches]
    ...
    def forward_with_dropout(self, x, verbose: bool = None):
        ...
        Same structure as forward(): hyper outputs [B, P+1], use token_head for patch_score.
        """
        if verbose is None:
            verbose = self.verbose
...
class ECGPatchIMNLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        logit, Wgen, tokens, _patch_score, T = self.model(x)
    ...
    def monte_carlo_dropout_inference(self, x, n_samples: int = 50):
        ...
        Returns:
          ...
          mean_heatmap: [B,12,T] mean **non-negative** heatmap across samples
          std_heatmap: [B,12,T] std of **non-negative** heatmap across samples
        ...
        for _ in range(n_samples):
            ...
            self.model.train()
            logit, Wpatch, _tokens, patch_score, _ = self.model.forward_with_dropout(x)
            self.model.eval()

            prob = torch.sigmoid(logit)
            patch_contrib = (Wpatch * patch_score)     # [B,P]
            patch_contrib = patch_contrib.abs()        # non-negative
            heatmap = patch_contrib.view(B, 12, T)     # [B,12,T]
...
def collect_wgen_tokens(model, loader, device, max_samples: int = None):
    ...
    for batch in loader:
        x, y = batch
        x = x.to(device)
        logit, Wgen, tokens, _patch_score, T = model.model(x)
        prob = torch.sigmoid(logit)
        B = x.size(0)
...
def visualize_pos_neg_to_pdf(model, dataset, device, pdf_path: str, sampling_rate: int,
    ...
    Uses **non-negative** contribution magnitudes from `model.explain`.
    """
    model.eval()
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            hm = heatmap.squeeze(0).detach().cpu().numpy()  # [12,T], non-negative
            x_np = x.detach().cpu().numpy()                 # [12,L]

            denom = np.max(hm) + 1e-6
            hm_disp = hm / denom  # [0,1]
            ...
            im = ax0.imshow(hm_disp, aspect="auto", vmin=0, vmax=1, cmap="Reds")
            ...
            for lead in range(12):
                ...
                contrib = hm_disp[lead]  # [T], 0..1
                for t in range(T):
                    a = float(contrib[t])
                    alpha = min(0.30, a * 0.30)
                    if alpha > 0:
                        start = t * stride
            ...
            axf.axis("off")
            axf.text(0, 0.5, "Red intensity = magnitude of contribution (non-negative, normalized per record).",
                     fontsize=10)
...
def visualize_topk_leads_patches_to_pdf(model, dataset, device, pdf_path: str, sampling_rate: int,
    ...
    Uses **non-negative** contribution magnitudes from `model.explain_with_topk`.
    """
    model.eval()
    if lead_names is None:
        lead_names = ["I","II","III","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6"]
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            for lead in range(12):
                ...
                for t in range(T):
                    ...
                    if alpha > 0:
                        ...
                        if is_top_patch:
                            ax.axvspan(start, end, alpha=alpha, color='red', linewidth=2, edgecolor='lime', zorder=1)
                        else:
                            ax.axvspan(start, end, alpha=alpha, color='red', linewidth=0)
            ...
            axf = fig.add_subplot(gs[13, 0])
            axf.axis("off")
            info_text = "Red intensity = magnitude of contribution (non-negative, normalized per record).\n"
...
def visualize_uncertainty_to_pdf(model, dataset, device, pdf_path: str, sampling_rate: int,
    ...
    Uses **non-negative** contribution magnitudes from `monte_carlo_dropout_inference`.
    """
    model.eval()
    if lead_names is None:
        lead_names = ["I","II","III","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6"]
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            mean_hm = mean_heatmap.squeeze(0).detach().cpu().numpy()  # [12,T], non-negative
            std_hm = std_heatmap.squeeze(0).detach().cpu().numpy()    # [12,T]
            x_np = x.detach().cpu().numpy()                           # [12,L]

            denom_mean = np.max(mean_hm) + 1e-6
            mean_hm_disp = mean_hm / denom_mean  # [0,1]
            ...
            # Mean heatmap
            ax0 = fig.add_subplot(gs[0, 0])
            im0 = ax0.imshow(mean_hm_disp, aspect="auto", vmin=0, vmax=1, cmap="Reds")
            ax0.set_yticks(range(12))
            ax0.set_yticklabels(lead_names)
            ax0.set_xlabel(f"Segments (window={window}, stride={stride}, fs={sampling_rate}Hz)")
            ax0.set_title(f"{tag} sample | true={int(y.item())} | P(MI)={mean_prob_val:.3f}±{std_logit_val:.3f} | idx={idx} | Mean Contribution Magnitudes")
            ...
            ax1.set_title(f"Uncertainty (Std of magnitudes) across {n_mc_samples} MC samples")
            ...
            for lead in range(12):
                ...
                mean_contrib = mean_hm_disp[lead]  # [T], 0..1
                ...
                for t in range(T):
                    ...
                    alpha_mean = min(0.30, a_mean * 0.30)
                    ...
                    if alpha_mean > 0:
                        ax.axvspan(start, end, alpha=alpha_mean, color="red", linewidth=0, label='Mean magnitude' if t == 0 else '')
            ...
            axf.axis("off")
            info_text = f"Mean contributions: Red intensity = magnitude (non-negative).\n"
...
def main():
    ...
    parser.add_argument("--wandb_name", type=str, default=None,
                        help="WandB run name (default: {script}[_{exp}]_{timestamp})")
    ...
    # Visualization
    parser.add_argument("--viz_pdf", type=str, default="viz_mi_vs_norm_test_pos.pdf", help="Output PDF filename")
    ...
    parser.add_argument("--viz_topk_pdf", type=str, default="viz_topk_test_pos.pdf", help="Output PDF filename for top-k visualization")
    ...
    parser.add_argument("--viz_uncertainty_pdf", type=str, default="viz_uncertainty_test_pos.pdf", help="Output PDF filename for uncertainty visualization")
    ...
    print(f"\n📊 Saving per-lead visualizations (non-negative contributions) to: {pdf_path}")
    ...
    if args.viz_topk:
        ...
        print(f"\n📊 Saving top-k leads/patches visualizations (non-negative contributions) to: {topk_pdf_path}")
    ...
    if args.viz_mc_dropout:
        ...
        print(f"\n📊 Saving Monte Carlo dropout uncertainty visualizations (non-negative contributions) to: {uncertainty_pdf_path}")
