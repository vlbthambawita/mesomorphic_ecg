"""
Gradio app for ECG Patch IMN (v8) with PTB-XL: load ECG from dataset path, show clinical
notes and ground truth, then predict, visualize, and ablate using the v8 model.

Usage: pip install gradio wfdb && python gradio_app_ptbxl_v8.py
"""
...
import script_28012026_v8_Selected_01_lightning_save_wgen_token as train
...
# Leave checkpoint path empty so user can choose any v8 checkpoint
DEFAULT_CKPT = ""
...
def load_ptbxl_meta(ptbxl_path: str, sampling_rate: int):
    ...
    Yf = Y[keep].copy()
    y_bin = np.array(
        [1.0 if "MI" in str(l) else 0.0 for l in Yf["diagnostic_superclass"].values],
        dtype=np.float32,
    )
...
def on_load_ptbxl(ptbxl_path: str, sampling_rate: int, state: Optional[dict]):
    ...
    choices = [
        f"{r['ecg_id']} | {r['gt']} | age {r['age']} {r['sex']}" for r in records
    ]
...
def predict_ptbxl(
    ...
    err = (
        "Select a record and Load PTB-XL first.",
        None,
        "—",
        "—",
        "—",
    ...
    with torch.no_grad():
        # v8 model forward: logit [B], Wpatch [B,P], tokens [B,P,D], patch_score [B,P], T (int)
        logit, Wpatch, tokens, patch_score, T = model.model(x_t)
        prob = torch.sigmoid(logit)
        # Per-patch signed contribution that sums to (logit - bias)
        patch_scores = Wpatch * patch_score  # [B,P]
        b = logit - patch_scores.sum(dim=1)  # [B]
...
def main():
    demo = gr.Blocks(title="ECG Patch IMN v8 – PTB-XL", theme=gr.themes.Soft())
    with demo:
        gr.Markdown(
            "# ECG Patch IMN v8 – PTB-XL\n"
            "Load ECGs from **PTB-XL** path. View **clinical notes** and **ground truth**, "
            "then run prediction, segment/lead importance, and ablation (v8 model)."
        )
        ...
        with gr.Row():
            ckpt = gr.Textbox(
                ...
                placeholder="path/to/best_v8.ckpt",
            )
        ...
        gr.Markdown(
            "PTB-XL path = dir with ptbxl_database.csv, scp_statements.csv. "
            "Test fold only. Red=MI, Blue=NORM. Red boxes=removed. (v8 model)"
        )
