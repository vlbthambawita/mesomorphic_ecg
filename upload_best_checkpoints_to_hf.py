#!/usr/bin/env python
"""
Upload best IMN checkpoints to Hugging Face Hub.

This script scans the following four main categories and four subcategories:

Main categories (local base directories):
- categorical_imn_100hz -> runs/imn_ecg_transition_net_GM_100Hz
- categorical_imn_500hz -> runs/imn_ecg_transition_net_GM
- single_linear_imn_100hz -> runs/imn_ecg_transition_net_GM_one_linear_100Hz
- single_linear_imn_500hz -> runs/imn_ecg_transition_net_GM_one_linear_500Hz
...
2. Within each run, finds checkpoint files named like:
   best-imn-epoch=E-val_auc=A.ckpt
...
4. Uploads the chosen checkpoint, together with that run's args.yaml and
   metrics.csv, into the specified Hugging Face model repo under:
...
Usage examples:
  python upload_best_checkpoints_to_hf.py --dry-run
  python upload_best_checkpoints_to_hf.py --repo-id SEARCH-IHI/mesomorphicECG
"""
...
CATEGORY_CONFIG: Dict[str, str] = {
    # 1. Categorical IMN 100Hz
    "categorical_imn_100hz": os.path.join(
        PROJECT_ROOT, "runs", "imn_ecg_transition_net_GM_100Hz"
    ),
    # 2. Categorical IMN 500Hz
    "categorical_imn_500hz": os.path.join(
        PROJECT_ROOT, "runs", "imn_ecg_transition_net_GM"
    ),
    # 3. Single-Linear IMN 100Hz
    "single_linear_imn_100hz": os.path.join(
        PROJECT_ROOT, "runs", "imn_ecg_transition_net_GM_one_linear_100Hz"
    ),
    # 4. Single-Linear IMN 500Hz
    "single_linear_imn_500hz": os.path.join(
        PROJECT_ROOT, "runs", "imn_ecg_transition_net_GM_one_linear_500Hz"
    ),
}
...
def find_best_checkpoint_in_run(run_dir: str) -> Optional[Tuple[str, int, float]]:
    """
    Find the best-imn checkpoint in a single run directory.
    ...
    for entry in os.listdir(run_dir):
        ...
        match = BEST_CKPT_PATTERN.match(entry)
        if not match:
            continue
...
def find_best_checkpoint_for_subcategory(
    base_dir: str, category_key: str, subcategory: str
    ...
    for run_name in sorted(os.listdir(subcat_dir)):
        ...
        found = find_best_checkpoint_in_run(run_dir)
        if not found:
            continue
    ...
    if best_overall is None:
        print(
            f"[WARN] No best-imn checkpoint found for "
...
def discover_all_best_checkpoints() -> List[BestCheckpoint]:
    ...
    for category_key, base_dir in CATEGORY_CONFIG.items():
...
def build_commit_operations(
    ...
    For each best checkpoint, we include:
      ...
      - metrics.csv from the same run directory (if present)
    """
    ...
    for bc in best_checkpoints:
        ...
        # 3) metrics.csv (optional)
        metrics_path = os.path.join(bc.run_dir, "metrics.csv")
        if os.path.isfile(metrics_path):
            metrics_repo_path = os.path.join(category_dir, subcat_dir, "metrics.csv")
            operations.append(
                CommitOperationAdd(
            ...
            print(f"[WARN] metrics.csv not found in run dir: {bc.run_dir}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Upload best IMN checkpoints to Hugging Face Hub."
    )
    parser.add_argument(
        "--repo-id",
        type=str,
    ...
    api.create_commit(
        ...
        commit_message="Add best IMN checkpoints for categorical and single-linear models (100Hz & 500Hz).",
    )
