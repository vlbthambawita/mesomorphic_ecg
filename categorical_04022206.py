...
CATEGORICAL / DUAL-OUTPUT VERSION (2 Output Nodes).

Based on: "Interpretable Mesomorphic Neural Networks" (NeurIPS 2024)

- Architecture: Deep Hypernetwork (CNN + Transition Decoder) -> Generates Weights W (K,C,L) and Bias b (K).
- Prediction: Logit_k = sum(Input * Generated_Weights_k) + Generated_Bias_k
- Loss: CrossEntropy + Lambda * L1_Norm(Generated_Weights)
- XAI: Class-Specific Intrinsic Attribution. 
       We visualize features supporting (Red) or refuting (Blue) the target class.

Run:
python imn_ptbxl_2d.py --path /path/to/ptbxl --sampling_rate 500 --epochs 20 --lambda_l1 1e-4 --viz_random --top_k_pos 3 --top_k_neg 3
"""
...
def get_top_pos_neg_segments_per_lead(impact: np.ndarray, window: int, stride: int, k_pos: int, k_neg: int) -> tuple[list[set[int]], list[set[int]]]:
    ...
    pos_segments: indices of top-k_pos segments with strongest positive contribution.
    neg_segments: indices of top-k_neg segments with strongest negative contribution.
    """
...
def imn_weights_to_segments(impact_12L: np.ndarray, window: int, stride: int) -> np.ndarray:
    """
    Aggregates point-wise feature attribution (Impact) into segments (Magnitude).
    """
...
def visualize_imn_to_pdf(model, dataset, device, pdf_path: str,
                         ...
                         top_k_neg: int = 0):
    """
    Visualizes IMN Feature Attributions for Categorical (Dual-Output) Model.
    
    If top_k_pos/top_k_neg > 0:
      - Uses signed aggregation.
      - Draws RED boxes for segments supporting the target class.
      - Draws BLUE boxes for segments refuting the target class.
    
    Target Class selection:
      - If sample is MI (y=1): Uses Class 1 weights. Red=Supports MI, Blue=Refutes MI.
      - If sample is NORM (y=0): Uses Class 0 weights. Red=Supports NORM, Blue=Refutes NORM.
    """
    model.eval()
    ...
    with PdfPages(pdf_path) as pdf:
        for tag, idx in selected:
            ...
            with torch.no_grad():
                ...
                # We visualize the evidence for the TRUE class.
                target_class_idx = y_int 
                
                # gen_w: [1, num_classes, 12, L]
                w_used = gen_w[0, target_class_idx, :, :].cpu().numpy()
                
            x_np = x.numpy()
            impact = w_used * x_np
            
            # 1. Heatmap: Shows Magnitude of Impact (where model looks)
            seg_hm = imn_weights_to_segments(impact, window=window, stride=stride)
            Tseg = seg_hm.shape[1]
            Lsig = x.shape[1]

            # 2. Segments: Show Direction of Impact (Pos/Neg)
            pos_segs, neg_segs = None, None
            if use_pos_neg_logic:
                pos_segs, neg_segs = get_top_pos_neg_segments_per_lead(impact, window, stride, top_k_pos, top_k_neg)

            # Metadata Strings
            target_class_name = pos_class_name if target_class_idx == 1 else "NORM"
            prob_str = f"P({pos_class_name})={prob_pos:.3f}" if target_class_idx == 1 else f"P(NORM)={prob_neg:.3f}"
            
            cmap = "Reds" # Base heatmap always shows activity magnitude
            ...
            expl_str = ""
            if use_pos_neg_logic:
                expl_str = f"| Box: Red=Supports {target_class_name}, Blue=Refutes {target_class_name}"
            
            ax0.set_xlabel(f"Segments (window={window}, stride={stride}, fs={sampling_rate}Hz)")
            ax0.set_title(f"IMN Class-Specific Explanation | {tag} | True={y_int} | {prob_str} | Target={target_class_name} {expl_str}")
            ...
            for lead in range(12):
                ...
                if use_pos_neg_logic:
                    # Draw Positive (Red)
                    if pos_segs:
                        for t in pos_segs[lead]:
                            draw_box(t, "red")
                    # Draw Negative (Blue)
                    if neg_segs:
                        for t in neg_segs[lead]:
                            draw_box(t, "blue")
                else:
                    # Fallback: Shading by magnitude (Legacy)
                    contrib = seg_hm[lead]
                    for t in range(Tseg):
                        a = float(contrib[t])
                        alpha = min(0.5, a * 0.6)
            ...
            axf.axis("off")
            footer_text = f"IMN Feature Attribution for Class {target_class_idx} ({target_class_name}). L1 Reg={lambda_l1}"
            if use_pos_neg_logic:
                footer_text += " | Red (+): Increases Logit. Blue (-): Decreases Logit."
...
def main():
    ...
    # NEW: Top-K Pos/Neg
    parser.add_argument("--top_k_pos", type=int, default=0,
                        help="Visualize top-k positive (Red) segments per lead.")
    parser.add_argument("--top_k_neg", type=int, default=0,
                        help="Visualize top-k negative (Blue) segments per lead.")
    ...
    parser.add_argument("--viz_negative_class", action="store_true",
                        help="Deprecated, logic handled automatically by top_k_pos/neg.")
